...
const CommunityActivityCard: React.FC<CommunityActivityCardProps> = ({ item, onUpvote }) => {
  ...
  const parsedContent = React.useMemo(() => {
    try {
      ...
      return { description: item.content, tip: '' };
    }
  }, [item.content]);
  ...
  return (
    <motion.div
      whileHover={{ y: -4 }}
      className="bg-white/70 backdrop-blur-sm rounded-2xl p-5 border border-emerald-100/50 shadow-sm hover:shadow-md transition-all group"
    >
      ...
      <div className="pl-6 mb-4">
        <p className="text-slate-600 text-sm italic mb-2">"{parsedContent.description}"</p>
        {parsedContent.tip && (
