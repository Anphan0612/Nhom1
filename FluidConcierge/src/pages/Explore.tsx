...
const Explore: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      {/* Hero Header */}
      <div className="pt-24 pb-32 px-6 relative overflow-hidden bg-emerald-950">
        ...
        {/* Overlay Gradient */}
        <div className="absolute inset-0 bg-gradient-to-b from-emerald-950/40 via-transparent to-background" />
        
        <div className="max-w-6xl mx-auto relative z-10 text-center">
          <motion.h1 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="text-4xl md:text-6xl font-bold text-white tracking-tight mb-6 font-display"
          >
          ...
          <motion.p 
            ...
            className="text-white/80 text-lg md:text-xl max-w-2xl mx-auto font-sans"
          >
          ...
          <div className="mt-12 max-w-3xl mx-auto relative group">
            <span className="material-symbols-outlined absolute left-6 top-1/2 -translate-y-1/2 text-emerald-400/50 group-hover:text-primary transition-colors">search</span>
            <input
              type="text"
              value={search}
              onChange={e => setSearch(e.target.value)}
              placeholder="Bạn muốn đi đâu hoặc làm gì? (vd: Phú Quốc, Beach...)"
              className="w-full pl-16 pr-8 py-6 rounded-3xl bg-white text-lg shadow-2xl focus:ring-4 focus:ring-primary/10 transition-all outline-none text-slate-900 font-sans"
            />
          </div>
        </div>
      </div>
